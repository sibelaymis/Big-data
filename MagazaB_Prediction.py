...
datasets = pd.read_csv('C:/Users/User/Desktop/test.csv', sep=';')
...
x1 = ['month','SAAT','gold','euro','dolar','temperature','special days','day_of_week']
trainingSetLabels =np.array(df[x1])


testSet = np.array(test['CIRO'])
w = np.transpose(np.matrix(testSet))
x2 = ['month','SAAT','gold','euro','dolar','temperature','special days','day_of_week']
